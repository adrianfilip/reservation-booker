...
private[infrastructure] case class ReservationServiceLive(
  ...
  override def addReservation(m: ReservationService.AddReservation): IO[
    ...
    for {
      ...
      room         <- roomRepository
                        .findById(m.roomId)
                        .someOrFail(ReservationDomainError.RoomNotFound(m.roomId))
      _            <-
  ...
  override def cancel(id: String): IO[ReservationDomainError.ReservationNotFound, Unit] =
    for {
      _ <- reservationRepository
             .findReservation(id)
             .someOrFail(ReservationDomainError.ReservationNotFound(id))
      _ <- reservationRepository.cancel(id)
    } yield ()

object ReservationServiceLive:

  val layer: ZLayer[ReservationRepository with BuildingRepository with RoomRepository, Nothing, ReservationService] =
    ZLayer {
      for {
      ...
      } yield new ReservationServiceLive(reservationRepository, buildingRepository, roomRepository)
    }
